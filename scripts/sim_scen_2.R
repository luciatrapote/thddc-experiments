...
# Scenario 2: High-Dimensional Clusters (p=200) with Contamination
...
accuracy_catell4  <- matrix(0, nrow = D, ncol = N)
...
time_catell4  <- matrix(0, nrow = D, ncol = N)
...
q1=10
mu1 <- rep(0, p)
Delta1 <- diag(c(seq(20,11,length=q1), rep(0.15, p - q1)))

# Second group
q2=5
Delta2 <- diag(c(seq(15,11,length=q2), rep(0.25, p - q2)))
...
for (d in 1:D)
{
  ...
  for (i in 1:N)
  {
    ...
    # ---------------------------------------------------------
    # 4) tHDDC CON CATELL (10,10)
    # ---------------------------------------------------------
    ini <- Sys.time()
    uX <- tHDDC_paralelizado_foreach(
      X, q=c(10,10), alpha=0.05, nstart = 250, cstep1=2, cstep2=25, nkeep=5,
      equal.weights=TRUE, restr.fact=c(5,3), zero.tol=1e-16,
      trace=0, opt="HARD", sol_ini_p=FALSE, sol_ini=NA,
      catell_Y=TRUE, catell_threshold=0.3, dim_max=40
    )
    ...
    time_catell4[d,i]     <- as.numeric(fin-ini, units="secs")
    accuracy_catell4[d,i] <- accuracy_clustering(uX$assig, true_labels)
    q_catell4[d, i, ] <- uX$q
    
    # ---------------------------------------------------------
    # 6.1) TCLUST (2 clusters; accurate trimming percentage; c=12 (default) used to ensure flexibility)
    # ---------------------------------------------------------
    ini <- Sys.time()
    ...
    # ---------------------------------------------------------
    # 6.2) TCLUST (2 clusters; accurate trimming percentage; c=50 used to ensure flexibility)
    # ---------------------------------------------------------
    ini <- Sys.time()
    tc <- tclust(X,K=2,alpha = 0.05, nstart = 250, cstep1=2, cstep2=25, nkeep=5,  equal.weights = FALSE, 
    ...
    # ---------------------------------------------------------
    # 6.3) TCLUST (2 clusters; accurate trimming percentage; c=150 used to ensure flexibility)
    # ---------------------------------------------------------
    ini <- Sys.time()
    tc <- tclust(X,K=2,alpha = 0.05, nstart = 250, cstep1=2, cstep2=25, nkeep=5,  equal.weights = FALSE, 
...
load("~/Simulaciones: Extending tclust to higher dimensions/sim_scen_2.RData")
